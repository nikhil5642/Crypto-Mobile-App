...
export const view: FC<Props<Navigator, NavigationParams>> = ({e, m, p}) => {
  function loadAppLevelInitialData() {
    DefaultPreference.get('defaultCurrency').then((data) => {
      e.of('loadDefaultCurrencyData').emit(data)
      console.log('sdfjkaklsdfjklas;fma', data)
    })
  }
  return (
    <Lifecycle onMount={loadAppLevelInitialData}>
      <NavigationContainer ref={navigationRef}>
        <Stack.Navigator
          ...
          <Stack.Screen name={Routes.CurrencyChangeScreen}>
            {({route}) => (
              <currencyChange.view
                e={e.of('currencyChange')}
                p={{...(p.params as CurrencyChangeParams), ...route.params}}
                m={m.currencyChange}
              />
            )}
          </Stack.Screen>
